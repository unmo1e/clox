...
// largest (stack top) to smallest (stack bottom)
static ObjUpvalue *captureUpvalue(Value *local) {
  ObjUpvalue *prevUpvalue = NULL;
  ObjUpvalue *upvalue = vm.openUpvalues;
  while(upvalue != NULL && local < upvalue->location) {
    prevUpvalue = upvalue;
...
static void closeUpvalues(Value *last) {
  while(vm.openUpvalues != NULL && last <= vm.openUpvalues->location) {
    ObjUpvalue *upvalue = vm.openUpvalues;
...
#define BINARY_OP(valueType, op)                                        \
  do {                                                                  \
    if(!IS_NUMBER(peek(0)) || !IS_NUMBER(peek(1))) {                    \
      runtimeError("Operands must be numbers for binary operation.");   \
